...
import {accI, AccTypes} from './Account.d'

const Account = ({data, Control} : AccTypes) => {
  const [vals, setVals] = useState<accI>(data)
        ...
        >
          ...
          <ModalBody>
            ...
            <Flex align="center" justify="space-between" wrap="wrap">
              ...
              <Switch defaultIsChecked={Boolean(data.enabled)} onChange={(e) => {setVals({...vals, enabled: +e.target.checked, day: 1, month: 1}); onClose()}}/>
            </Flex>
              ...
              <Stack spacing={1}>
                ...
                <InputLeftAddon children="Mes" />
                  <NumberInput 
                    ...
                    max={12}
                    onChange={(v) => {setVals({...vals, month: Number(v)})}}>
                    <NumberInputField />
                    <NumberInputStepper>
                      <NumberIncrementStepper />
                      <NumberDecrementStepper />
                ...
                <InputLeftAddon children="Día" />
                  <NumberInput 
                    ...
                    keepWithinRange={true}
                    max={31}
                  onChange={(v) => {setVals({...vals, day: Number(v)})}}>
                    <NumberInputField />
                    <NumberInputStepper>
                      <NumberIncrementStepper />
                      <NumberDecrementStepper />
